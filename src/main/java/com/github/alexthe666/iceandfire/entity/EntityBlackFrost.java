...
public class EntityBlackFrost extends EntityDragonBase {
   ...
   public EntityBlackFrost(World worldIn) { this(IafEntityRegistry.BLACK_FROST.get(), worldIn);}
    ...
    public void riderShootFire(Entity controller) {
        if (this.getRNG().nextInt(5) == 0 && !this.isChild()) {
            if (this.getAnimation() != ANIMATION_FIRECHARGE) {
                ...
                EntityBlackFrostCharge entitylargefireball = new EntityBlackFrostCharge(
                        IafEntityRegistry.BLACK_FROST_CHARGE.get(), world, this, d2, d3, d4);
    ...
    protected SoundEvent getAmbientSound() {
        return IafSoundRegistry.ICEDRAGON_ADULT_IDLE;
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return IafSoundRegistry.ICEDRAGON_ADULT_HURT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return IafSoundRegistry.ICEDRAGON_ADULT_DEATH;
    }

    @Override
    public SoundEvent getRoarSound() {
        return  IafSoundRegistry.ICEDRAGON_ADULT_ROAR ;
    }

    @Override
    public Animation[] getAnimations() {
        return new Animation[]{IAnimatedEntity.NO_ANIMATION, EntityDragonBase.ANIMATION_EAT, EntityDragonBase.ANIMATION_SPEAK, EntityDragonBase.ANIMATION_BITE, EntityDragonBase.ANIMATION_SHAKEPREY, EntityBlackFrost.ANIMATION_TAILWHACK, EntityBlackFrost.ANIMATION_FIRECHARGE, EntityBlackFrost.ANIMATION_WINGBLAST, EntityBlackFrost.ANIMATION_ROAR};
    }
